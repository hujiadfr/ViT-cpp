...
namespace transformer {
    template<typename T, int DIM, int DIM_HID, int ENC_LAYER_CNT, int KERNEL_WIDTH, int N_CLASS>
    struct transformerParameter{
        Conv2dParameter<T, KERNEL_WIDTH, DIM> patch_p;
        EncoderParameter<T, DIM, DIM_HID, ENC_LAYER_CNT> encoder_p;
    ...
    public:
        static void forward(std::array<std::array<std::array<T, FIG_WIDTH>, FIG_WIDTH>,IN_CH> &input_fig,
                            ...
                            transformerParameter<T, DIM, DIM_HID, ENC_LAYER_CNT, KERNEL_WIDTH, N_CLASS> &p) {
        auto *patches = new std::array<std::array<T, DIM>,DEP+1>{};
        auto *tmp = new std::array<std::array<T, DIM>,DEP+1>{};
        std::cout<<"Start Patch Embedding"<<std::endl;
        ...
        std::cout<<"Start MLP head"<<std::endl;
